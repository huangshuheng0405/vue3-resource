...
var ShapeFlags = /* @__PURE__ */ ((ShapeFlags22) => {
  ...
  ShapeFlags22[(ShapeFlags22['FUNCTIONAL_COMPONENT'] = 2)] =
  ...
  ShapeFlags22[(ShapeFlags22['COMPONENT_SHOULD_KEEP_ALIVE'] = 256)] =
    'COMPONENT_SHOULD_KEEP_ALIVE'
  ShapeFlags22[(ShapeFlags22['COMPONENT_KEPT_ALIVE'] = 512)] =
...
function computed(getterOrOptions) {
  let onlyGetter = isFunction2(getterOrOptions)
  let getter, setter
  if (onlyGetter) {
...
function traverse(
  ...
  if (!isObject22(source)) return source
  if (depth) {
...
function doWatch(source, callback, { deep, depth, immediate }) {
  ...
  else if (isFunction2(source)) getter = source
  let oldValue
  let clean
  const onCleanup = (fn) => {
    clean = () => {
